...
export class K8sBaselineStack extends cdk.Stack {
  constructor(scope: Construct,
    ...
    */
    ...
    // Conditional check for all supporting resources
    ...
    */
    ...
    OpenTelemetry is configured to emit all public available metrics for ContainerInsights, to reduce
    CloudWatch Metric cost customize the ADOT collector
    ...
    */
    // version is based on default metric server manifest see file for origin,
    // does not require clean up function as namespace service account does not reply on IAM Role creation dependency.
    const manifestMetricServer = yaml.loadAll(fs.readFileSync('manifests/metricServerManifest.yaml', 'utf-8'), null, { schema: yaml.JSON_SCHEMA }) as [Record<string, any>];
  ...
  cleanManifest(file: string) {
    const manifest = yaml.loadAll(fs.readFileSync(file, 'utf-8'), undefined, { json: true }) as [Record<string, any>];
